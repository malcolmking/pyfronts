...
def pad_longitude(data,npts):
	'''
	Pads data longitudinally, so we can extend things beyond
	the edge of the data for global datasets.Requires 2-D data
	in lat x lon format.
	'''
	if 'lon' in data.dims:
		pad_data=xr.concat([data[:,-1*npts:],data,data[:,:npts]],dim='lon')
		temp_long=pad_data.lon
		temp_long.data[:npts]=pad_data.lon[:npts]-360
		temp_long.data[-1*npts:]=pad_data.lon[-1*npts:]+360
	else:
		pad_data=xr.concat([data[:,-1*npts:],data,data[:,:npts]],dim='longitude')
		temp_long=pad_data.longitude
		temp_long.data[:npts]=pad_data.longitude[:npts]-360
		temp_long.data[-1*npts:]=pad_data.longitude[-1*npts:]+360
	return pad_data
...
def frontfields2(data,ua,va,threshold=-0.3e-10):
	# returns a field where zero crossings indicate fronts, along with an indicator of front speed
	# and magnitude
	# INPUTS: data - field to find fronts on (2d xarray)
	#         ua - zonal winds on same surface as data (xarray)
...
def front(data,u,v,threshold_i=-0.3e-10,threshhold_s=1.5,numsmooth=3,smooth_kernel=np.ones((3,3))/9,minlength=250,searchdist=1.5):
	...
	clines=linejoin(cpts,minlength=minlength,searchdist=searchdist)
	wlines=linejoin(wpts,minlength=minlength,searchdist=searchdist)
	slines=linejoin(spts,minlength=minlength,searchdist=searchdist)
